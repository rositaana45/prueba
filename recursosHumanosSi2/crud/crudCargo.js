...
$(document).ready(function(){
	...
	$(document).on('submit', '#emp-SaveForm', function() {
	   ...
	   $.post("../control/Ccargo.php", $(this).serialize())
        .done(function(data){
			$("#dis").fadeOut();
			...
			$(".content-loader").fadeOut('slow', function()
			 {
				...
				$(".content-loader").load('cargoEditar.php?edit_id='+edit_id);
				$("#btn-add").hide();
				$("#btn-view").show();
			});
		}
	...
	$(document).on('submit', '#emp-UpdateForm', function() {
	   ...
	   $.post("../control/cargoActualizar.php", $(this).serialize())
        .done(function(data){
			$("#dis").fadeOut();
			$("#dis").fadeIn('slow', function(){
			     $("#dis").html('<div class="alert alert-info">'+data+'</div>').show(200).delay(2500).hide(200);
				 ...
				 {
					$("body").fadeOut('slow');
					window.location.href="../formulario/cargo.php";
				 });				 
		     });	
		});   
	    return false;
