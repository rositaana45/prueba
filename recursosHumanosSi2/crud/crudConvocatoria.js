...
$(document).ready(function(){
	...
	$(document).on('submit', '#emp-SaveForm', function() {
	   ...
	   $.post("../control/Cconvocatoria.php", $(this).serialize())
        .done(function(data){
			$("#dis").fadeOut();
			...
			$(".content-loader").fadeOut('slow', function()
			 {
				...
				$(".content-loader").load('convocatoriaEditar.php?edit_id='+edit_id);
				$("#btn-add").hide();
				$("#btn-view").show();
			});
		}
	...
	$(document).on('submit', '#emp-UpdateForm', function() {
	   ...
	   $.post("../control/convocatoriaActualizar.php", $(this).serialize())
        .done(function(data){
			$("#dis").fadeOut();
			$("#dis").fadeIn('slow', function(){
			     $("#dis").html('<div class="alert alert-info">'+data+'</div>').show(200).delay(2500).hide(200);
				 ...
				 {
					$("body").fadeOut('slow');
					window.location.href="../formulario/convocatoria.php";
				 });				 
		     });	
		});   
	    return false;
