...
$(document).ready(function(){
	...
	$(document).on('submit', '#emp-SaveForm', function() {
	   ...
	   $.post("../control/Crequisitos.php", $(this).serialize())
        .done(function(data){
			$("#dis").fadeOut();
	...
	$(".delete-link").click(function()
	{
		...
		{
			$.post('requisitosEliminar.php', {'del_id':del_id}, function(data)
			{
				parent.fadeOut('slow');
			});	
		}
			...
			$(".content-loader").fadeOut('slow', function()
			 {
				...
				$(".content-loader").load('requisitosEditar.php?edit_id='+edit_id);
				$("#btn-add").hide();
				$("#btn-view").show();
			});
		}
	...
	$(document).on('submit', '#emp-UpdateForm', function() {
	   ...
	   $.post("../control/requisitoActualizar.php", $(this).serialize())
        .done(function(data){
			$("#dis").fadeOut();
			$("#dis").fadeIn('slow', function(){
			     $("#dis").html('<div class="alert alert-info">'+data+'</div>').show(200).delay(2500).hide(200);
